...
func HandleMessage(s *session.Session, c *gateway.MessageCreateEvent) {
	...
	message, found := strings.CutPrefix(message, config.BotConfig.DefaultPrefix)
	if !found {
		return
	}
	...
	HandleCommand(s, c, command, args)
}

func HandleCommand(s *session.Session, mce *gateway.MessageCreateEvent, command string, args []string) {
...
func HandleInteractions(s *session.Session, e *gateway.InteractionCreateEvent) {
	var resp api.InteractionResponse
	switch d := e.Data.(type) {
	case *discord.CommandInteraction:
		if d.Name == "stats" {
			emb := SlashStats(d.Options[0].String())
