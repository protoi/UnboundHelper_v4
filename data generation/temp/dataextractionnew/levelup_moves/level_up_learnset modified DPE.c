...
#pragma endregion level_up_move
...
#pragma region POKEMON_NAMES
	...
	const struct LevelUpMove *const gLevelUpLearnsets[] =
		{
#pragma region level up mapping
...
#pragma endregion movename
	...
	{
		// all this is done to remove the occurence of the weird numbers after missing no entry
		char temp[7]; // size of "(null)" is 1 + 4 + 1 (+1) // the last 1 is the null terminator
		sprintf(temp, "%s", pokemon_name_mapping[pokemonID]);
		// printf(temp);
		char *nullstr = "(null)";
		if (strcmp(temp, nullstr) == 0)
			continue;

		int l, m; // level and move
		int flag = 0;
		if (gLevelUpLearnsets[pokemonID][0].lvl == 0x0 && gLevelUpLearnsets[pokemonID][0].move == 0xFFFF)
			continue;
		fprintf(file, "\t\"%s\" : [\n", pokemon_name_mapping[pokemonID]); // key
		char *c = ",\n";
		// printf("%d -> %s\n", pokemonID, pokemon_name_mapping[pokemonID]);
		for (int i = 0;; i++) // value
		{
			l = gLevelUpLearnsets[pokemonID][i].lvl, m = gLevelUpLearnsets[pokemonID][i].move;
			if (m == 0xFFFF) // end of moveset
				break;
