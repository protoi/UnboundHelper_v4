...
void structPrinter(FILE *file, int moveindex, struct BattleMove moveinfo)
{
	fprintf(file, "{\"target\": \"%s\",\"type\": \"%s\",\"split\": \"%s\",\"flags\": %d,\"z_move_effect\": %d,\"effect\": %d,\"power\": %d,\"accuracy\": %d,\"pp\": %d,\"secondaryEffectChance\": %d,\"priority\": %d,\"z_move_power\": %d},",
			moveinfo.target,
			moveinfo.type,
			moveinfo.split,
			moveinfo.flags,
...
int main()
{
	FILE *file = fopen("./battle_move.json", "a");
	if (file == NULL)
	{
	...
	for (int i = 0; i < 923; i++)
	{
		structPrinter(file, i, gBattleMoves[i]);
		printf("%d\n", i);
	}
	fprintf(file, "]");
